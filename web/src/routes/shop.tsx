  ...
  head: () => ({
    meta: [
      { title: "Shop groceries online — Martify" },
      {
        name: "description",
        content:
          "Browse fresh produce, bakery, dairy, pantry and more. Filter by category, diet and price, then check out in minutes.",
      },
      { property: "og:title", content: "Shop groceries online — Martify" },
      {
        property: "og:description",
        content: "Browse thousands of grocery items with AI-curated picks and same-day delivery.",
      },
        ...
        <div className="mt-4 flex flex-col lg:flex-row lg:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
              {loading ? (
                <InlineLoader label="Loading live catalogue" />
              ) : (
        ...
        <aside className="hidden lg:block w-64 shrink-0">
          <div className="sticky top-32 space-y-6">
            ...
            <div className="rounded-3xl bg-primary-soft/60 border border-primary/10 p-5">
              ...
              <Link
                to="/assistant"
                className="mt-3 inline-flex h-9 items-center rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground"
              >
                Open assistant
              </Link>
            </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {usingFallback && !loading && (
            <p className="mt-5 text-xs text-muted-foreground">
              Showing the preview catalogue while the live store reconnects.
            </p>
          )}
        </div>
      </div>
