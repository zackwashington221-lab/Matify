...
function Assistant() {
  ...
  async function send(event?: FormEvent) {
    ...
    try {
      ...
      const text = error instanceof Error ? error.message : "The assistant is unavailable right now.";
      setMessages((current) => [...current, { role: "ai", text: text === "AI shopping is not configured. Set GEMINI_API_KEY." ? "The shopping assistant is being set up. Please try again shortly." : text }]);
    } finally {
      setLoading(false);
    }
  }

  return <StoreLayout><div className="mx-auto max-w-3xl px-4 lg:px-8 py-8 pb-32">
    <div className="space-y-4">
      {messages.map((message, index) => <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
        <div className={`max-w-[90%] ${message.role === "user" ? "" : "space-y-2"}`}>
          <div className={`px-4 py-3 text-[14px] leading-relaxed ${message.role === "user" ? "bg-primary text-primary-foreground rounded-3xl rounded-br-md" : "bg-card border border-border rounded-3xl rounded-bl-md"}`}>
            {message.role === "ai" && <Sparkles className="mr-2 inline size-4 text-primary" />}{message.text}
          </div>
          {message.advice?.recommendations.length ? <AdviceCard advice={message.advice} add={add} /> : null}
        </div>
      </div>)}
      {loading && <div className="text-sm text-muted-foreground"><Sparkles className="mr-2 inline size-4 text-primary" />Building your basket…</div>}
    </div>
...
function AdviceCard({ advice, add }: { advice: ShoppingAdvice; add: (id: string, qty?: number) => void }) {
  const [added, setAdded] = useState(false);
  const addAll = () => { advice.recommendations.forEach((item) => add(item.product.slug, item.qty)); setAdded(true); };
  return <div className="rounded-3xl bg-card border border-border p-4 shadow-soft"><div className="flex items-center justify-between"><span className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">AI basket</span><span className="font-semibold">${advice.total.toFixed(2)}</span></div><div className="mt-3 space-y-2">{advice.recommendations.map((item) => <div key={item.product._id} className="flex items-center gap-3"><Link to="/product/$id" params={{ id: item.product.slug }} className={`grid size-10 place-items-center rounded-xl bg-gradient-to-br ${item.product.gradient || "from-emerald-100 to-lime-100"}`}>{item.product.emoji || "🛒"}</Link><div className="min-w-0 flex-1"><div className="text-sm font-semibold truncate">{item.product.name} × {item.qty}</div><div className="text-xs text-muted-foreground truncate">{item.reason}</div></div><button type="button" onClick={() => add(item.product.slug, item.qty)} className="text-xs font-semibold text-primary">Add</button></div>)}</div><button type="button" onClick={addAll} className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-semibold text-primary-foreground">{added ? <Check className="size-4" /> : <Plus className="size-4" />}{added ? "Basket added" : "Add basket to cart"}</button></div>;
}
