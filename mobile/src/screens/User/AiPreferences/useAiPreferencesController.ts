...
export default function useAiPreferencesController() {
  ...
  return { values: { title: "AI preferences", description: "Make Freshly's recommendations more personal.", items: [{ title: "Healthy swaps", detail: preferences.healthySwaps ? "Enabled" : "Disabled" }, { title: "Budget alerts", detail: preferences.budgetAlerts ? "Weekly grocery budget: $" + preferences.weeklyBudget : "Disabled" }, { title: "Dietary preferences", detail: preferences.dietaryPreferences.length ? preferences.dietaryPreferences.join(", ") : "No preferences selected" }] }, functions: {} };
}
